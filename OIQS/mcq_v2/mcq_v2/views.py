...
def login_view(request):
	...
	if request.method=="POST":    #for not authenticated users.
		if form_class.is_valid():
			...
			if status=="Teacher":
			    ...
			    print("Error")											# for authenticated users.
...
def teacher(request):
	...
	multivalue=zip(quiz_object,quescount)
	print(multivalue)
	context = {
		'Teacher':teacher,
	...
	if request.POST:
		...
		for student in object_1:
			if student.quiz==quiz_object:
				...
				return redirect('/information')
				# return redirect(reverse(result,kwargs={'id':student.pk}))
		...
		messages.info(request,"Thanks for your time.")
		return redirect('/information')
		# return redirect(reverse(result,kwargs={'id':lb1.pk}))
	if request.user.is_authenticated:
		if request.user.sprofile_set.all().first().start_time==0:
			obj=sprofile.objects.get(user=request.user)
			obj.start_time=0
			obj.save()
		# else:
		# 	if request.user.is_authenticated:
